...
public class StudentSEEPerformance {
	...
	public StudentSEEPerformance() {
		initialize();	
	}
	
	public StudentSEEPerformance(String usn) {
		this.USN = usn;
		initialize();	
	}
	...
	public void initialize()  {
	  ...
	  //PANEL 1 CREATION - HOME BUTTON CONTENTS
	    ...
	    try {
			...
			String query = "Select s.subcode,s.sname,s.scredits,ia.FinalIA,ia.SEE,ia.TotalMrks,ae.status from iamarks ia,subjects s,academiceligibility ae where ia.USN = ae.USN and ia.subcode = ae.subcode and ia.subcode=s.subcode and s.sem=? and ia.usn=?;";
		 ...
		 table.getTableHeader().setForeground(Color.WHITE);
			...
			tableMode1.addColumn("FINAL-IA Mrks (40)");
			tableMode1.addColumn("SEE Marks (60)");
			tableMode1.addColumn("Total Marks (100)");
			...
			while(set.next()) {
				...
				int finalia = set.getInt(4);
				int seemrks = set.getInt(5);
				int totalmrks = set.getInt(6);
				String status = set.getString(7);
							
				Object data1[] = {subcode,name,credits,finalia,seemrks,totalmrks,status};
...
//			sp.setViewportBorder(BorderFactory.createLineBorder(Color.black,2));
	 ...
	 table1.getTableHeader().setForeground(Color.WHITE);
		...
		tableMode2.addColumn("FINAL-IA Mrks (40)");
		tableMode2.addColumn("SEE Marks (60)");
		tableMode2.addColumn("Total Marks (100)");
		tableMode2.addColumn("STATUS");
...
//						panel2.setVisible(false);
					...
					try {
						...
						while(set2.next()) {
							...
							int finalia = set2.getInt(4);
							int seemrks = set2.getInt(5);
							int totalmrks = set2.getInt(6);
							String status = set2.getString(7);
										
							Object data2[] = {subcode,name,credits,finalia,seemrks,totalmrks,status};
	...
	public static void main(String[] args) {
		StudentSEEPerformance ssp = new StudentSEEPerformance("4SF20CS004");
	}
}
