...
public class UpdateFacultyRecord {

	//JCOMPONENTS VARIABLE DECLARATION 
	JFrame frame;
	JLabel l0,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l15,l16;
	JTextField tf1,tf2,tf4,tf5,tf6,tf7,tf8;
	JComboBox cb1,cb2,cb3,cb5;
	JButton b1,b2,b3;
	...
	public UpdateFacultyRecord() {
		initialize();
	}
	
	public UpdateFacultyRecord(String adname) {
	...
	public void initialize() {
		...
		l2 = new JLabel("FACULTY  RECORD   UPDATION  PORTAL");
		l2.setBounds(460,45,800,100);
		l2.setFont(new Font("Algerian",Font.BOLD,35));
		
		l3 = new JLabel("Enter   Updated   Faculty   Information   ...",SwingConstants.CENTER);
	    ...
	    l16.setBounds(40,330,150,100);
	    l16.setFont(new Font("Times New Roman",Font.PLAIN,28));
	    
	    tf8 = new JTextField("enter last name");
		tf8.setBounds(185,350,275,60);
		...
		tf8.addFocusListener(new FocusAdapter() {
		    public void focusGained(FocusEvent fe) {	
		    	if(tf8.getText().trim().equals("enter last name"))
		        	tf8.setText("");
		    }
		    public void focusLost(FocusEvent e) {
		    	if(tf8.getText().trim().equals(""))
		            tf8.setText("enter last name");		    	
		    }
		});
	    ...
	    cb5.addKeyListener(new KeyAdapter() {
	    	public void keyPressed(KeyEvent ke) {
	    		if (ke.getKeyCode() == KeyEvent.VK_UP){
					tf6.grabFocus();
		        }
	    ...
	    tf2 = new JTextField("enter faculty ID");
		...
		tf2.addFocusListener(new FocusAdapter() {
		    public void focusGained(FocusEvent fe) {	
		    	if(tf2.getText().trim().equals("enter faculty ID"))
		        	tf2.setText("");
		    }
		    public void focusLost(FocusEvent e) {
		    	if(tf2.getText().trim().equals(""))
		            tf2.setText("enter faculty ID");		    	
		    }
		});
		tf2.addActionListener(new ActionListener() {
		      public void actionPerformed(ActionEvent e) {
					...
					try {
						fid = Integer.parseInt(tf2.getText());
						
						Connection con = CP.createCon();
						String query = "select FFNAME,FLNAME,FDESIGNATION,FSALARY,FPHNO,FCITY,FGENDER,FDOB,FEMAIL,did from FACULTY where fid=?;";
						...
						if(set.next()) {
							...
							desg = set.getString(3);
							pay = set.getString(4);
							phno=set.getString(5);
							...
							JOptionPane.showMessageDialog(frame,"Error !! No Records Found...Please enter a valid Faculty ID ","Error Message",JOptionPane.ERROR_MESSAGE);
							tf2.setText("");
							return;
						}
						...
						//SALARY-PAY
						if(pay.equals("50k-100k")) {
							cb2.setSelectedIndex(1);
						}
						else if(pay.equals("100k-150k")) {		
							cb2.setSelectedIndex(2);
						}
						else if(pay.equals("150k-200k")) {
							cb2.setSelectedIndex(3);
						}
						else if(pay.equals("200k-300k")) {
						...
						//DESIGNATION
						if(desg.equals("Assistant Professor")) {
							cb3.setSelectedIndex(1);
						}
						else if(desg.equals("Associate Professor")) {
							cb3.setSelectedIndex(2);
						}
						else if(desg.equals("Professor")) {
...
//						se.printStackTrace();
						JOptionPane.showMessageDialog(frame,"Error !!  Please enter a valid Faculty ID ","Error Message",JOptionPane.ERROR_MESSAGE);
						tf2.setText("");
					}
...
//						e.printStackTrace();
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				if(stg.equals("SAVE") == true) {
					FacultyTable ft = new FacultyTable(fid,fname,lname,desg,pay,phno,city,gender,dob,email,did);
					try {
						FacultyTableOperationsDB.updatefacultytable(ft);
						JOptionPane.showMessageDialog(frame,"Successful !! Faculty record - " + fname + " updated successfully","Success Message",JOptionPane.INFORMATION_MESSAGE);
	...
	public static void main(String[] args) {
		UpdateFacultyRecord sr = new UpdateFacultyRecord(); 
	}
